...
{
   public function create($lastname, $firstname, $email, $phone, $password, $date, $address) {
    $password = password_hash($password, PASSWORD_DEFAULT);
    $name = $lastname . " " . $firstname;
    
    // Sửa lại câu SQL với backtick chính xác
    $sql = "INSERT INTO `khach_hang` 
            (`Ten_Khach_Hang`, `Email`, `So_Dien_Thoai`, `Mat_Khau`, `Ngay_Dang_Ky`, `Dia_Chi`, `status`, `Trang_Thai`) 
            VALUES (?, ?, ?, ?, ?, ?, 1, 1)";
    ...
    // Bind parameters
    $stmt->bind_param("ssssss", $name, $email, $phone, $password, $date, $address);
    ...
    public function updateUserByAdmin( $id,$name, $phone)
    {
        $password = password_hash($password, PASSWORD_DEFAULT);
        $sql = "UPDATE `khach_hang` SET `Ten_Khach_Hang`='$name',`So_Dien_Thoai`='$phone' WHERE `ID_Khach_Hang`=$id";
        $result = mysqli_query($this->con, $sql);
        return $result;
    }
